...
function applyDeviceFilters() {
  ...
  if (_deviceFilter === 'approved') list = list.filter(d =>  d.approved || d.status === 'approved');
  if (_deviceFilter === 'pending')  list = list.filter(d => d.status === 'pending');
  if (_deviceFilter === 'ignored')  list = list.filter(d => d.status === 'ignored');
  if (q) list = list.filter(d =>
    (d.mac_src  ||'').toLowerCase().includes(q) ||
    (d.ip_src   ||'').toLowerCase().includes(q) ||
...
// Hook render functions to cache data
